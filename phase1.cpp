...
int main(int argc, char *argv[])
{
    ...
    for (size_t i = 1 + neighbours.size(); i < fd_count; i++)
    {
        ...
        const char *hello = (to_string(id) + '_' + to_string(unique_private_id) + '_' + port_temp + '#').c_str();
        //cout<<"blah"<<sizeof hello<<"blah"<<endl;
        send(pfds[i].fd, hello, 12, 0);
    }
    ...
    for (;;)
    {
        if (total_listened == neighbours.size())
            break;
        // std::cout << "abc";
            ...
            { // We got one!!
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        const char *buf_temp = "END#";
                        send(pfds[i].fd, buf_temp, sizeof buf_temp, 0);
                        del_from_pfds(pfds, i, &fd_count);
